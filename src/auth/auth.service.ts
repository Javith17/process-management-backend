...
export class AuthService {
    ...
    async sendVendorOTP(sendOtp: SendVendorOTPDto) {
        try {
            const vendor = await this.vendorRepo.findOne({ where: { vendor_mobile_no1: sendOtp.phone_number } });
            if (!vendor) {
                return { message: "Vendor not registered" }   
            } else {
